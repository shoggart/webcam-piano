        ...
        for (var i = 0; i < this.notePos.length; i++) {
            for (var j = 0; j < this.notePos[i].length; j++) {
                ...
                if (this.noteState[i][j] > 0) {
                    var alpha = this.noteState[i][j] * 200 * sin(frameCount / 10) + 100;
                    alpha = constrain(alpha, 150, 255);
                    var c1 = color(constrain(255 * sin(frameCount / 10), 100, 255),
                        0,
                        constrain(255 * tan(frameCount / 10), 100, 255),
                        alpha);
                    var c2 = color(0, constrain(255 * sin(frameCount / 10), 100, 255), 0, alpha);
